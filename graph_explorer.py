...
import logging
my_logger = logging.getLogger("graph_exploration")
...
class ProfileNode:
    ...
    def get_followers(self):
        try:
            if not self.is_private:
                my_logger.info('looking for followers of ' +  self.username + '...')
    ...
    def get_following(self):
        try:
            if not self.is_private:
                my_logger.info('looking for what ' + self.username + ' follows ...')
...
def sample_connected_nodes(connected_nodes, max_relatives = 1000):
    n = min(len(connected_nodes), max_relatives)
...
def get_ProfileNode_from_id(profile_id, insta_session):
    ...
    try:
        ...
        my_logger.info('process node ' + curr_profile.username + '...')
    except Exception as e:
        my_logger.exception(e)
...
def explore_neighborhood_BFS(root, insta_session, max_nb_explored_profiles = 10000, max_relatives = 100):
    ...
    while (len(profiles_to_explore) > 0) and (len(neighboring_network) < max_nb_explored_profiles):
        try:
            my_logger.info('length of profiles to explore = ' + str(len(profiles_to_explore)) + ' limited at ' + str(len(neighboring_network)))
